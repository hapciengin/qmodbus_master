...
QString TcpIpSettingsWidget::getIpAddress() const
{
    return ui->edNetworkAddress->text();
}

int TcpIpSettingsWidget::getPort() const
{
    return ui->edPort->text().toInt();
}

void TcpIpSettingsWidget::setConnectionParameters(const QString &ip, int port)
{
    ui->edNetworkAddress->settext(ip);  // setText yerine settext kullanıyoruz
